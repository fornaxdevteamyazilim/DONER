'use strict';
app.controller('BarcodeDefinitionCtrl', BarcodeDefinitionCtrl);
function BarcodeDefinitionCtrl($rootScope, $scope, $log, $modal, $filter, SweetAlert, Restangular, ngTableParams, toaster, $window, $stateParams, $location, $translate, ngnotifyService, $element) {
    $rootScope.uService.EnterController("BarcodeDefinitionCtrl");
    var inp = this;
    $scope.item = {};
    $scope.objectType = 'BarcodeDefinition';
    inp.search = '';
    if ($scope.Date) {
         $scope.Date;
    } else {
    ...
    $scope.translate = function () {
        $scope.trBarcodeTypeID = $translate.instant('main.BARCODETYPEID');
        $scope.trInventoryUnitID = $translate.instant('main.INVENTORYUNITID');
        $scope.trMask = $translate.instant('main.MASK');
        $scope.trMultipliyer = $translate.instant('main.MULTIPLIYER');
        $scope.trQuantity = $translate.instant('main.QUANTITY');
        $scope.trQuantityEndDigit = $translate.instant('main.QUANTITYENDDIGIT');
        $scope.trQuantityStartDigit = $translate.instant('main.QUANTITYSTARTDIGIT');
    ...
    $scope.addItem = function () {
        inp.tableParams.data.push({ InventoryUnitID: $scope.Date });
    };
    ...
    $scope.$on('$destroy', function () {
        ...
        deregistration1();
        $element.remove();
        $rootScope.uService.ExitController("BarcodeDefinitionCtrl");
    });
};
