...
function storecarrierCtrl($rootScope, $scope, $log, $modal, Restangular, ngTableParams, SweetAlert, toaster, $window, $translate, $element) {
    ...
    $scope.translate = function () {
        $scope.trStore = $translate.instant('main.STORE');
        $scope.trCarrier = $translate.instant('main.CARRIER');
        $scope.trFilter = $translate.instant('main.FILTER');
        $scope.trisDefault = $translate.instant('main.ISDEFAULT');
        $scope.isDispachEnabled = $translate.instant('main.ISDISPATCHENABLED');
        $scope.trCommands = $translate.instant('main.COMMANDS');
    }
    $scope.translate();
    var deregistration = $scope.$on('$translateChangeSuccess', function (event, data) {// ON LANGUAGE CHANGED
    ...
    $scope.saveData = function (data) {
        if (data.restangularized) {
            ...
            data.post().then(function (res) { sc.tableParams.reload(); toaster.pop('success', "Kaydedildi.", 'Saved.'); });
            data.get();
        }
    };
    $scope.FormKeyPress = function (event, rowform, data, index) {
    ...
    $scope.cancelForm = function (rowform) {
        rowform.$cancel();
        if (!sc.tableParams.data[sc.tableParams.data.length - 1].restangularized) {
            $scope.cancelremove(sc.tableParams.data.length - 1, 1);
            toaster.pop('warning', "İptal edildi !", 'Insert cancelled !');
        } else {
            toaster.pop('warning', "İptal edildi !", 'Edit cancelled !');
        }
    };
    sc.tableParams = new ngTableParams({
        page: 1,
        count: 10,
        //sorting: {
        //    name: 'asc'
    ...
    $scope.carriers = [];
    $scope.loadEntities('Carrier', 'carriers');
    $scope.filters = [];
    $scope.loadEntities('filter', 'filters');
    $scope.stores = [];
    ...
    $scope.removeItem = function (index) {
        SweetAlert.swal({
            ...
            if (isConfirm) {
                if (sc.tableParams.data[index].fromServer) {
                    sc.tableParams.data[index].remove();
                }
                sc.tableParams.data.splice(index, 1);
    ...
    $scope.addItem = function () {
        sc.tableParams.data.push({});
    };
    var deregistration1 = $scope.$watch(angular.bind(sc, function () {
        return sc.search;
    }), function (value) {
    ...
    $scope.$on('$destroy', function () {
        ...
        deregistration1();
        $element.remove();
        $rootScope.uService.ExitController("storecarrierCtrl");
    });
};
